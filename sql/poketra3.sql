...
CREATE OR REPLACE VIEW v_stock AS (
    SELECT m.idMatiere, m.nommatiere, SUM(mvmt.qteEntree) - SUM(mvmt.qteSortie) AS qteRestante
    FROM matiere m
    JOIN mouvement_stock mvmt ON mvmt.idMatiere = m.idMatiere
    GROUP BY m.idMatiere, m.nommatiere
);

CREATE OR REPLACE VIEW v_listeproduit AS (
    SELECT 
    v_formule.*,
    m.prix,
    m.prix * v_formule.quantite AS prixtotal
    FROM v_formule
...
--edit tsy mandeha 
create or replace view v_prix as(
select nomlook,nomtaille,nomtype,sum(prixtotal) 
from v_listeproduit 
group by nomlook,nomtaille,nomtype
);
