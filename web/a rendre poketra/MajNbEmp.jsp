...
<%
    Vector [] listeTailles = (Vector []) request.getAttribute("listeTailles");
...
<body>
    <form style="max-width: 700px;margin: 70px;margin-left: 112px;" role="form" action="MajNbEmp" method="post">
        <div class="form-group mb-3" style="font-size: 36px;text-align: center;color: rgb(177,28,19);"><label class="form-label" style="font-family: 'Abril Fatface', serif;font-weight: bold;text-decoration:  underline;"><span style="color: rgb(48, 47, 47);">Formulaire de MAJ du nombre employes:</span></label></div>
        <div class="form-group mb-3"><label class="form-label" style="border-color: rgb(242,7,7);">Tailles:&nbsp;</label>
            <select class="form-select" id="tomSelect2" required="" name="idTaille" style="background: rgb(108,108,108);">
                <% for(int i = 0; i < listeTailles.length; i++) { %>
                <option value="<%= listeTailles[i].get(0) %>"><%= listeTailles[i].get(6) %> <%= listeTailles[i].get(2) %> <%= listeTailles[i].get(4) %></option>
                <% } %>
            </select>
        </div>
